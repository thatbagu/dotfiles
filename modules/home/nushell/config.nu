...
def load-bash-env [] {
    # Create a temporary file to store env vars
    let tmp_file = (mktemp)
    
    # Use bash to export all variables in a format we can parse
    bash -c $"printenv | sort > ($tmp_file)"
    
    # List of protected variables that shouldn't be modified
    let protected_vars = [
        '_'
        'pwd'
        'oldpwd'
    ...
    | each { |line|
        if ($line | str length) > 0 {
            ...
            if ($parts | length) == 2 {
                ...
                if ($key | str downcase) not-in $protected_vars {
...
# Function to reload PATH from bash
def reload-path [] {
    let bash_path = (bash -c "echo $PATH")
    try { load-env { PATH: $bash_path } }
}
...
$env.config = {
    ...
    pre_execution: [{ 
        if $env.BASH_ENV? == null {
            ...
            # Load *_LOAD variables
            let load_vars = ($env | columns | where { |col| $col ends-with "_LOAD" })
            for var in $load_vars {
                let path_value = ($env | get $var)
                let target_var = ($var | str replace "_LOAD" "")
                
                if ($path_value | path exists) {
                    let content = (open $path_value | str trim)
                    let env_record = { $target_var: $content }
                    load-env $env_record
                }
            }
